...
import { Box, BoxShadow, Canvas, Fill, FitBox, Paint, Path, rect, rrect, Shadow, vec } from '@shopify/react-native-skia'
...
const BoxShadows = () => {
  ...
  const rct = useMemo(() => {
    const c = vec(width / 2, r); // x: 214, y: 150
  ...
  return (
    <Canvas style={{ flex: 1 }} mode="continuous" debug>
      ...
      <Box box={rct} color="white">
        <BoxShadow dx={dx} dy={dx} blur={10} color="#93b8c4" />
      </Box>
      <FitBox src={rect(0, 0, 24, 24)} dst={rect(50, 350, 300, 300)}>
        <Paint>
          <Shadow dx={10} dy={10} blur={1} color="red" inner />
